...
## Context
LifeOS v1 kept Google Calendar strictly read-only. The CLI guide said "do not add Google Calendar write commands," and the `lifeos-tools-v2` scratch notes went further: keep any future writes create-only on a dedicated reminders calendar, and "never create guest events or modify attendee lists from this tool."
