...
## Ready for Human QA
- [ ] **Update your private `.env`, then confirm `doctor` passes.** *(Blocking — Google features are broken until this is done.)* The secret files moved into `secrets/`, and the four absolute path vars in your `.env` still point at the old root locations. Edit `lifeos-tools/secrets/.env` (the `.env` itself moved too) and change these four to add `/secrets`:
  - `GOOGLE_CALENDAR_CREDENTIALS_PATH="$CONFIGS/lifeos-tools/secrets/google-credentials.json"`
  - `GOOGLE_CALENDAR_TOKEN_PATH="$CONFIGS/lifeos-tools/secrets/google-token.json"`
  - `GOOGLE_ACCOUNTS_PATH="$CONFIGS/lifeos-tools/secrets/google-accounts.json"`
  - `LIFEOS_PEOPLE_ALIASES_PATH="$CONFIGS/lifeos-tools/secrets/people-aliases.json"`

  No `google-accounts.json` edit is needed — its per-account `credentials_path`/`token_path` are bare relative names, which the code now resolves against `secrets/`. After editing, run `lifeos doctor` and confirm all-OK. Verified via env-override that this is the only remaining step: with those four paths pointed at `secrets/`, doctor already returns exit 0 with every file found.
