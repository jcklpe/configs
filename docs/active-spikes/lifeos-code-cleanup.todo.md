...
## To Do
- [ ] Move the Python helpers into `lib/` (not a separate `python/` folder — language-agnostic library) and re-point their invocations. Note: after modularization the `python3 ${SCRIPT_DIR}/google-*.py` calls now live in `lib/google.sh`, not `lifeos.sh`; plus 4 sites in `tests/`. Run the tests after.
