...
- [x] **Wrote `skills/commit-work/SKILL.md`.** Description leads with triggers and carries the violation keywords (`git add`, `commit -a`, `add -f`, `--amend`, `gitall`, `push`). Body covers the pathspec rule and its justification, scoping and the "and" test, the completion boundary, the `git add -f` trap, the five-way failure-mode table, the trailer, message format, an eleven-row rationalization table, and a red-flags list ending on "explaining to yourself why this particular case is different."
...
- [x] **Wired `commit-work` into `run-project-spike`.** Added a `## Committing Spike Work` section (completion boundary, `.todo.md` edit rides along, QA approvals batched per approval event, never push) and an `### The Archival Commit` subsection under archiving (synthesis not rollup; dead ends leave no trace in the tree). Kept it **policy-neutral**: `run-project-spike` is a global skill used in repos that may forbid agent commits, so it says to follow the repo's own policy and, *if the repo says nothing, not to commit and not to offer*. This repo's `0003` is what switches it on. `run-project-spike` writes no commit logic and no messages; it hands `commit-work` the slug and gets out of the way.
